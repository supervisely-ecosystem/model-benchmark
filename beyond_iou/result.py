...
from beyond_iou.templates import table_template, markdown_template
...
class Result:
    ...
    def from_evaluator(cls, evaluator):
        ...
        dataframe.to_csv("output/result_df.csv", index=True)
    ...
    def create_report(self, directory, exist_ok=False, img_format="png"):
        ...
        renorm_bar_chart_path = os.path.join(
            directory, f"renorm_bar_chart.{img_format}"
        )
        ...
        renorm_bar_chart = self.make_renormalization_bar_chart(
            path=renorm_bar_chart_path
        )
    ...
    def make_renormalization_bar_chart(self, path=None):
        keys = [
            "E_boundary_oU_renormed",
            "E_extent_oU_renormed",
            "E_segment_oU_renormed",
        ]
    ...
    def make_class_chart(self, path=None):
        ...
        labels = [
            f"{l} ({s*100:4.1f})"
            for l, s in zip(self.dataframe.index, self.dataframe["IoU"])
        ]
        ...
        plt.axhline(
            self.dataframe.loc["mean"]["IoU"] * 100,
            linestyle="-",
            color="w",
            linewidth=0.5,
        )
