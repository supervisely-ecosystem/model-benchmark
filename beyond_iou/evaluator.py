...
from beyond_iou.result import Result
from beyond_iou.utils import (
    one_hot,
...
class Evaluator:
    def __init__(
        ...
        if 0 < self.boundary_width < 1:
            ...
            print(
                f"Using relative boundary width of {self.boundary_width} * image diagonal."
            )
    ...
    def evaluate(self, loader: Iterable):
        ...
        for pred, gt, img_name in tqdm(loader, total=len(loader), smoothing=0):
        ...
        with open("output/img2iou.json", "w") as file:
            json.dump(self.img2iou, file)

        with open("output/cell_img_names.json", "w") as file:
        ...
        np.save("output/confusion_matrix.npy", normalized_confusion_matrix)
    ...
    def evaluate_sample(self, pred, gt, img_name):
        ...
        intersection = np.logical_and(pred_one_hot, gt_one_hot).sum(axis=(1, 2))
        union = np.logical_or(pred_one_hot, gt_one_hot).sum(axis=(1, 2))
        union = union.astype(float)
        union[union == 0] = 1e-6
        img_iou = np.mean(intersection / union)
        self.img2iou[img_name] = float(img_iou)
    ...
    def update_results(self, sample_results):
        ...
        for error_name, error_code in ERROR_CODES.items():
            self.results[error_name] += (
                sample_results["main_results"] == error_code
            ).sum(axis=(1, 2))
